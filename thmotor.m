...
classdef thmotor < handle 
    ...
    %
    % Example:
    % a=thmotor.listdevices   % List connected devices
    % m1=thmotor              % Create a thmotor object  
    ...
    % one degree is approx 1919.8 'clicks'
    ...
    %
    % Known Issues:
    % 1. If thmotor object gets deleted or corrupted it is sometimes necessary to
    % restart Matlab
    %
    % Version History:
    ...
    methods
        function h=thmotor()  % Instantiate motor object
            thmotor.loaddlls; % Load DLLs (if not already loaded)
        end
...
%                     h.motorSettingsNET=h.deviceNET.GetMotorConfiguration(serialNo); % Get motorSettings via .NET interface
    ...
    methods (Static)
        ...
        function loaddlls() % Load DLLs
            if ~exist(thmotor.DEVICEMANAGERCLASSNAME,'class')
                try   % Load in DLLs if not already loaded
                    NET.addAssembly([thmotor.MOTORPATHDEFAULT,thmotor.DEVICEMANAGERDLL]);
                    NET.addAssembly([thmotor.MOTORPATHDEFAULT,thmotor.GENERICMOTORDLL]);
                    NET.addAssembly([thmotor.MOTORPATHDEFAULT,thmotor.DCSERVODLL]); 
                    NET.addAssembly([thmotor.MOTORPATHDEFAULT,thmotor.INTEGSTEPDLL]); 
